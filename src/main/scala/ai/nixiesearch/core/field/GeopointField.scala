...
object GeopointField extends FieldCodec[GeopointField, GeopointFieldSchema, Array[Byte]] {
  ...
  override def makeDecoder(spec: GeopointFieldSchema, fieldName: String): Decoder[Option[GeopointField]] = {
    Decoder.instance(
      _.as[Option[Geopoint]].map(_.map(gp => GeopointField(fieldName, gp.lat, gp.lon)))
    )
  }
