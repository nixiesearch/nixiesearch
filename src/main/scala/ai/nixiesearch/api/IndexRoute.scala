...
case class IndexRoute(indices: Map[String, IndexBuilder]) extends Route with Logging {
  import IndexRoute._
  ...
  def index(request: Request[IO], indexName: String): IO[Response[IO]] = for {
    ...
    writer   <- IO.fromOption(indices.get(indexName))(new Exception(s"index $indexName is missing"))
