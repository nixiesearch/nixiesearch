...
class DateTimeFilterTest extends SearchTest with Matchers {
  ...
  it should "filter over gt match" in withIndex { index =>
    {
      val days    = DateTimeFieldCodec.parseString("2024-02-01T00:00:00Z").toOption.get
  ...
  it should "filter over gt/lt match" in withIndex { index =>
    {
      val days1   = DateTimeFieldCodec.parseString("2024-01-01T00:00:00Z").toOption.get
      val days2   = DateTimeFieldCodec.parseString("2024-03-01T00:00:00Z").toOption.get
      val results = index.search(filters =
        Some(
          Filters(include =
            Some(
