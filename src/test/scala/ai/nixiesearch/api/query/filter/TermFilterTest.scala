...
class TermFilterTest extends SearchTest with Matchers {
  ...
  val index = List(
    Document(List(TextField("id", "1"), TextField("color", "red"))),
    Document(List(TextField("id", "2"), TextField("color", "white"), TextField("color2", "light white"))),
    Document(List(TextField("id", "3"), TextField("color", "red")))
  )

  it should "select terms" in new Index {
    val results = search(filters = Filter(include = Some(TermPredicate("color", "white"))))
    results shouldBe List("2")
  }

  it should "select terms with space" in new Index {
    val results = search(filters = Filter(include = Some(TermPredicate("color2", "light white"))))
    results shouldBe List("2")
  }
