...
import ai.nixiesearch.core.Field.{DateField, IdField, IntField, LongField, TextField}
...
class DateFilterTest extends SearchTest with Matchers {
  ...
  val docs = List(
    Document(List(IdField("_id", "1"), DateField.applyUnsafe("date", "2024-01-01"))),
    Document(List(IdField("_id", "2"), DateField.applyUnsafe("date", "2024-02-01"))),
    Document(List(IdField("_id", "3"), DateField.applyUnsafe("date", "2024-03-01")))
  )
