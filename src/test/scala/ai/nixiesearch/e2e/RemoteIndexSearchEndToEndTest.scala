...
class RemoteIndexSearchEndToEndTest extends AnyFlatSpec with Matchers {
  ...
  it should "write index to s3" in {
    val (master, masterShutdown)   = Index.forIndexing(mapping, CacheConfig(), conf.inference).allocated.unsafeRunSync()
  ...
  it should "search from s3" in {
    val (slave, slaveShutdown)       = Index.forSearch(mapping, CacheConfig(), conf.inference).allocated.unsafeRunSync()
